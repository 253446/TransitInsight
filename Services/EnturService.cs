...
public class EnturService
{
    ...
    {
        var departures = await GetLiveDeparturesForStopAsync(enturId);

        foreach (var departure in departures)
        {
            departure.StopPlaceId = stopPlaceId;
        }
    ...
    public async Task<List<EnturStopSearchResult>> GetNearbyStopsAsync(double latitude, double longitude)
    {
        ...
        var url =
            $"https://api.entur.io/geocoder/v1/reverse?point.lat={lat}&point.lon={lon}&boundary.circle.radius=1.5&size=15&lang=no";
    ...
    public async Task<List<Departure>> GetLiveDeparturesForStopAsync(string enturId)
    {
        ...
        foreach (var call in calls.EnumerateArray())
        {
            ...
            var delay = (int)Math.Max(0, (expected - aimed).TotalMinutes);

            var line = call.GetProperty("serviceJourney").GetProperty("line");
